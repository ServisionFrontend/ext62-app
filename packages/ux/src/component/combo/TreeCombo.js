...
*/
Ext.define('Ext.ux.component.combo.TreeCombo', {
	...
	initComponent: function() {
		...
		me.tree = Ext.create('Ext.tree.Panel', {
			...
			listeners: {
				load: function(store, records) {
					...
					me.fireEvent('afterload', me, store, records);
		...
		if (me.tree.getRootNode() && me.tree.getRootNode().get('checked') != null) me.multiselect = true;
	...
	initEvents: function() {
		...
		me.on('expand', function() {
			...
			me.setValue(val);
			me.ids = val.length > 0 ? val.split(',') : val;
		});
		me.on('collapse', function(that) {
			if (me.multiselect) {
				that.tree.getRootNode().cascade(function(curNode) {
